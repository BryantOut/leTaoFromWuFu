<template>
  <div class="searchList">
    ...
    <!-- 排序结构 -->
    <div class="lt_sorder">
      <a href="javascript:;" data-order="time">上架时间
        <span class="fa fa-angle-down"></span>
      </a>
      <a href="javascript:;" data-order="price" @click='handlePrice' :class='{"active":isActive===0}'>价格
        <span :class="{'fa':true,'fa-angle-down':hasPrice,'fa-angle-up':!hasPrice}"></span>
      </a>
      <a href="javascript:;" data-order="num" @click='handleNum'  :class='{"active":isActive===1}'>销量
        <span :class="{'fa':true,'fa-angle-down':hasNum,'fa-angle-up':!hasNum}"></span>
      </a>
      <a href="javascript:;" data-order="discount">折扣
        <span class="fa fa-angle-down"></span>
      </a>
    </div>
    <mt-loadmore :auto-fill='false' :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore">
      <router-link :to="'/productDetail/'+item.id" class="lt_proBox" v-for='item in resultList' :key='item.id'>
...
export default {
  ...
  mounted () {
    ...
    getSearchList({ proName: this.key, page: this.page, pageSize: this.pageSize, price: this.priceSortType }).then(res => {
