<template>
    <div class="search">
        <div class="searchArea">
            ...
            <mt-button size="small" @click='searchProduct'>搜索</mt-button>
            <!-- <mt-button size="small" @click='$router.push({"path":"/searchList/"+searchKey})'>搜索</mt-button> -->
            <mt-cell :to="{'name': 'SearchList', params: {'key': item}}" :title="item" v-for='(item,index) in histotyArr' :key='index'><span @click.prevent.stop ='delHistory(index)'>x</span></mt-cell>
...
export default {
  ...
  methods: {
    ...
    searchProduct () {
      ...
      this.$router.push({'name': 'SearchList', params: {'key': this.searchKey}})
    },
    getHistory () {
