<template>
    <div class="productDetail">
        <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore">
            <mt-swipe :auto="4000">
                <mt-swipe-item v-for='item in proObj.pic' :key='item.id'>
        ...
        <div class="lt_footer">
            <router-link to="/cart" class="fa fa-shopping-cart"></router-link>
            <mt-button type="primary">立即购买</mt-button>
            <mt-button type="danger" @click="addCart">加入到购物车</mt-button>
        </div>
    </div>
</template>
    ...
    addCart () {
      addCart({
        ...
        if (res.error === 400) {
          ...
          this.$router.push({ name: 'Login', params: {'source': `/productDetail/${this.proId}`} })
        } else {
          // 这里注意小写
          this.$messagebox.confirm('添加成功，请问是否查看购物车?').then(action => {
            //   跳转到购物车页面
